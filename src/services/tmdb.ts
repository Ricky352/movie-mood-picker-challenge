...
import type { Movie, TMDBDiscoverResponse, TMDBMovie, TMDBMovieDetails } from "../types/movie"
...
const tmdbFetch = async <T>(endpoint: string, params: Record<string, string | number> = {}): Promise<T> => {
 ...
 */
export const fetchMoviesByMood = async (moodConfig: MoodConfig): Promise<TMDBMovie[]> => {
...
export const searchMovies = async (query: string): Promise<TMDBMovie[]> => {
  const data = await tmdbFetch<TMDBDiscoverResponse>("/search/movie", { query })
  return data.results
}
